...
export default function LoginForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/adminLogin`, {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                setMessage(result.message || "Invalid or missing credentials...");
                setTimeout(() => {
                    setMessage("");
                    setMessageType("");
                }, 3000);
    ...
    return (
        <div className="h-screen flex items-center justify-center">
            <form onSubmit={handleSubmit} className="flex flex-col gap-3 bg-white p-8 w-[450px] rounded-2xl font-sans shadow">
                ...
                {/* Submit button */}
                <button type="submit" className="mt-5 bg-gray-900 hover:bg-gray-800 text-white font-medium rounded-lg h-12 w-full cursor-pointer transition">
                    Sign In
                </button>
